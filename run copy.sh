...
python rescale.py ${prefix} --image #he-raw.jpg ->  he-scaled.jpg
python preprocess.py ${prefix} --image #he-scaled.jpg -> he.jpg

# extract histology features
python extract_features.py ${prefix} --device=${device} #output->embeddings-hist-raw.pickle
...
python select_genes.py --n-top=${n_genes} "${prefix}cnts.tsv" "${prefix}gene-names.txt" #output : Top n genes -> gene-names.txt
